...
│              │              │              │          │      │
│              ▼              ▼              ▼          ▼      │
│        text-embedding-v3  Qdrant      qwen-plus   生成回答   │
│          (1024维)        (Docker)      (重排序)              │
└─────────────────────────────────────────────────────────────┘
```
...
export class ReActPlanner {
  ...
  parseIntent(input: string): Intent {
    ...
    if (lower.includes('3d') || lower.includes('模型') || lower.includes('展示')) {
    ...
    if (intent.type === 'generate_3d') {
      return {
        steps: [
          ...
          { tool: 'optimize_prompt', input: { analysis: { type: 'reference', stepId: 'step_1', path: '.' } } },
          { tool: 'generate_3d', input: { prompt: { type: 'reference', stepId: 'step_2', path: 'prompt' } } },
          { tool: 'quality_check', input: { modelUrl: { type: 'reference', stepId: 'step_3', path: 'modelUrl' } } }
        ]
      }
    }
...
export class PromptOptimizer {
  optimize(analysis: ProductAnalysis, options?: OptimizeOptions): OptimizedPrompt {
    // 1. 确定风格
...
export class PromptEvaluator {
   ...
   */
  async evaluate(prompt: string, analysis: ProductAnalysis): Promise<QualityScore> {
    try {
      // LLM 评估（更准确）
  ...
  private async llmEvaluate(prompt: string, analysis: ProductAnalysis): Promise<QualityScore> {
...
请从以下维度打分（1-10），返回 JSON：
{
  ...
  private heuristicEvaluate(prompt: string, analysis: ProductAnalysis): QualityScore {
    // 基于规则的快速评估
    let score = 5
...
export async function POST(request: Request) {
  ...
  for (const item of prompts) {
    ...
    // 优化后评估
    const afterScore = await evaluator.evaluate(optimized.prompt, item.analysis)
