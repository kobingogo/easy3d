...
| 模型类型 | 模型名称 | 用途 | 说明 |
|----------|----------|------|------|
| **文本生成** | qwen-plus | 规划、提示词生成 | 平衡性能与成本 |
...
export class Tracer {
  ...
  endStepSuccess(
    ...
    if (metadata?.tokensUsed) {
      ...
      step.cost = this.costCalculator.calculate(metadata.modelUsed, metadata.tokensUsed)
...
class BailianClient {
  ...
  async generate(options: {
    ...
    const { model = 'qwen-plus', messages, tools, temperature = 0.7, responseFormat } = options
...
export const analyzeProductTool: Tool<AnalyzeProductInput, AnalyzeProductOutput> = {
  type: 'function',
  function: {
...
export const optimizePromptTool: Tool<OptimizePromptInput, OptimizePromptOutput> = {
  type: 'function',
  function: {
  ...
  handler: async (input, context) => {
    ...
    try {
      ...
      const result = await llm.generate({
        ...
        messages: [{
          role: 'user',
          content: buildPromptGenerationMessage(analysis, knowledge, template, platform)
        }],
...
export const exportModelTool: Tool = {
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        modelUrl: { type: 'string', description: '3D 模型 URL' },
        format: { type: 'string', enum: ['glb', 'gif', 'mp4'], description: '导出格式' }
      },
...
export class WorkflowEngine {
  ...
  async execute(workflow: Workflow): Promise<WorkflowResult> {
    ...
    for (let i = 0; i < workflow.steps.length; i++) {
      ...
      // 执行步骤
      const stepResult = await this.executeStep(step, results, workflow.id, tracer)
  ...
  private resolveInput(input: StepInput, previousResults: Map<string, StepResult>): any {
    switch (input.type) {
      case 'static':
        return input.value
      case 'reference':
